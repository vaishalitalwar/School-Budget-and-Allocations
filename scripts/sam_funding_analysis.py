...
def get_funding_source(sam_text):
    ...
    funding_sources = [re.findall(r'\b(?:state|city|federal|multiple|unsure)\b', choice.text.strip().lower())[0] for choice in response.choices]
    funding_counts = {source: funding_sources.count(source) for source in ['state', 'city', 'federal', 'multiple', 'unsure']}
