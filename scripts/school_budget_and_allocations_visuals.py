...
allocations = pd.read_csv(
    "https://raw.githubusercontent.com/Erik-Brown01/School-Budget-and-Allocations/streamlit_visuals/data/district%205/allocation_district_5.csv"
)
allocations = allocations.drop(columns=["Unnamed: 0"])
allocations["amountNum"] = allocations["amount"].apply((lambda x: moneyToFloat(x)))

budget = pd.read_csv(
    "https://raw.githubusercontent.com/Erik-Brown01/School-Budget-and-Allocations/streamlit_visuals/data/district%205/budget_district_5.csv"
)
...
allocationCategories = pd.read_csv(
    "https://raw.githubusercontent.com/Erik-Brown01/School-Budget-and-Allocations/streamlit_visuals/data/allocation_category_lookup.csv"
)

enrollment = pd.read_csv(
    "https://raw.githubusercontent.com/Erik-Brown01/School-Budget-and-Allocations/streamlit_visuals/data/district%205/district_5_demographic_data.csv"
)
...
enrollment["Total Enrollment"] = (
    enrollment["Total Enrollment"].str.replace(",", "").astype(float)
)
...
school_code = st.selectbox(
    "Select School Code", unique_school_codes, index=default_value
)
...
totalsMelt = totalsPivot.melt(
    id_vars=["location_code", "type"], var_name="year", value_name="amount"
)
...
# Display Line chart
budgets_allocations_chart = (line + dots).properties(
    width=600, height=400, title="Budget vs Allocations"
)
...
funding_per_category_filtered = funding_per_category[
    funding_per_category["location_code"] == school_code
]
...
enrollment["3K-PreK_enrollment"] = (
    enrollment["Grade 3K"] + enrollment["Grade PK (Half Day & Full Day)"]
)
enrollment["K-5_enrollment"] = (
    enrollment["Total Enrollment"] - enrollment["3K-PreK_enrollment"]
)

# Make Pre-K Enrollment Pivot
preK_pivot = (
    enrollment[["DBN", "fiscal_year", "3K-PreK_enrollment"]]
    .pivot(index=["DBN"], columns=["fiscal_year"], values="3K-PreK_enrollment")
    .reset_index()
)
...
elementary_pivot = (
    enrollment[["DBN", "fiscal_year", "K-5_enrollment"]]
    .pivot(index=["DBN"], columns=["fiscal_year"], values="K-5_enrollment")
    .reset_index()
)
...
relevant_funding_per_category_pivot = funding_per_category_pivot[
    funding_per_category_pivot["allocation_category_group"].isin(
        ["Fair Student Funding", "Preschool"]
    )
]
relevant_funding_per_category_pivot = relevant_funding_per_category_pivot.rename(
    columns={
...
relevant_funding_per_category_pivot = relevant_funding_per_category_pivot.drop(
    columns=[2022]
)
...
enrollment_and_funding_pivot = pd.concat(
    [enrollmentPivot, relevant_funding_per_category_pivot]
)
...
enrollment_and_funding_melted["year"] = enrollment_and_funding_melted["year"].astype(
    int
)
...
enrollment_and_funding_melted[["Type", "value"]] = enrollment_and_funding_melted.apply(
    update_type_and_value, axis=1, result_type="expand"
)

# Filter to selected school code
filtered_enrollment_and_funding_melted = enrollment_and_funding_melted[
    enrollment_and_funding_melted["DBN"] == school_code
]
...
line_types = [
    "3K-PreK Enrollment",
    "K-5 Enrollment",
    "Fair Student Funding / 10,000",
    "Preschool Funding / 10,000",
]
...
multiline_chart = alt.layer(
    *[
        base.transform_filter(alt.datum["Type"] == line_type).mark_line()
...
total_enrollment_pivot = total_enrollment.pivot(
...
staff_and_enrollment_melt = staff_and_enrollment.melt(
    id_vars=["location_code", "type"], var_name="year", value_name="amount"
)

# Modify Total Enrollment values
staff_and_enrollment_melt.loc[
    staff_and_enrollment_melt["type"] == "Total Enrollment", "amount"
] /= 10

# Filter for school_code
filtered_staff_and_enrollment_melt = staff_and_enrollment_melt[
    staff_and_enrollment_melt["location_code"] == school_code
]
...
staff_and_enrollment_chart = (
    ...
    .properties(
        width=600,
        height=400,
        title="Total Enrollment / 10, Teacher, and Paraprofessionals",
    )
)
...
budget_per_staff_filtered = budget_per_staff[
    budget_per_staff["location_code"] == school_code
]

# Create and display bar chart for each FY
for fiscal_year in budget_per_staff_filtered["fiscal_year"].unique():
    budget_per_staff_filtered_temp = budget_per_staff_filtered[
        budget_per_staff_filtered["fiscal_year"] == fiscal_year
    ]
    budget_per_staff_chart = (
        ...
        .properties(
            width=600,
            height=400,
            title="Average Budget for Teachers and Paraprofessionals FY"
            + str(fiscal_year),
        )
    )
